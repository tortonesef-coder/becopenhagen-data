...
**Just finished**
- 14 September: the fleet's repair data was purged at Fede's request (ten June/July tickets and their 41 `action_log` lines, deleted in the fleet app), and the same 41 lines were removed from this archive's copy: `archive/action_log/part-20260810T140240Z.parquet` rewritten with DuckDB without them (2562 rows to 2521, same schema, no other row changed, the high-water mark 2566 kept), swapped in with one rename; the whole `action_log` archive then read 4257 rows, max id 4302. `bc.repairs` empties itself at the next hourly rebuild. No code change.
...
- **One deliberate exception to append-only: the repair trial** (14 September 2026, Fede). The fleet's June/July repair tickets were purged "like they never existed" so no future repair statistic counts them; the 41 matching `action_log` rows (`repair_ticket`, `ticket_updated`, `ticket_resolved`) were removed from the archive copy too. Repair data in every statistic starts empty from 14 September 2026.
...
- **2026-09-14 (the repair lines removed from the archive):** part of the fleet's repair purge (see the fleet's context, same date). Rehearsed on a copy in the scratchpad: `COPY (SELECT * FROM read_parquet(part) WHERE action NOT IN (...) ORDER BY id) TO new (FORMAT PARQUET, COMPRESSION ZSTD)`, then `DESCRIBE` equal on both files and `EXCEPT` both ways empty apart from the 41. Live: written next to the original as a dotfile, verified again, `mv` over the original. `archive-logs.sh` reads its high-water mark from `MAX(id)` over the Parquet glob, which the rewrite left at 4302 overall (2566 in that file), so the next hourly append continues unchanged. `bc.repairs` reads the fleet snapshot and is empty from the 15:35 rebuild on; `update-catalog-stats.sh` will show it at 0 rows. No entries between 11 August and this one; the 12 September consolidation touched documents only.
