...
# Hand-curated catalog tables (guide invoices, business facts, commission).
# They live in catalog_store.duckdb, which the agent's read-only connection
# never opens, so without this copy they are data nobody can ask about.
# Not fatal: these are supporting tables, and a warehouse without them is worse
# but still correct, whereas refusing to publish would lose an hour of bookings.
CATALOG_SQL="$(dirname "$0")/../sql/build-catalog-views.sql"
if [ -f "$CATALOG_SQL" ] && ! duckdb "$TMP" < "$CATALOG_SQL" > /dev/null; then
  log "WARNING: catalog tables not copied into the warehouse. Invoices and business facts will not be queryable this hour."
fi
